...
async fn process_batch(
    ...
    match perform_transformation(pool, source_type, corr_id, batch_id).await {
        Ok(()) => {
...
async fn perform_transformation(
    ...
    let inserted_global_rows = transaction
        .execute(global_query, &[&source_type, &corr_id, &batch_id])
        .await?;
    ...
    Logger::info(&format!(
        "Completed transformation for {} batch {}: Global({}) Deleted({})",
        source_type, corr_id, inserted_global_rows, deleted_rows
    ));
