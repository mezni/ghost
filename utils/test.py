...
df_operators = df_combined.sort_values(by=['Country', 'Operator'])
...
df_operators['operator_name_upper'] = df_operators['Operator'].str.upper()
...
df_joined = df_operators.merge(
    df_networks,
    left_on=['country_upper','operator_name_upper'],
    right_on=['country_upper','brand_upper'],
    how='left'
)

#df_joined=df_joined[['Country', 'Operator',  'PLMN_x', 'tech_2g', 'tech_3g','tech_lte','ISO','TADIG']]

#df_joined = df_joined.sort_values(by=['Country', 'Operator'])

print (df_joined.head())

df_joined.to_csv('xxxx.csv', index=False)


df_remain = df_joined[df_joined['TADIG'].isnull()]

print (df_remain.head())

df_remain.to_csv('yyyy.csv', index=False)