...
const INSERT_BARRED_OPERATOR_ALERTS: &str = r#"
INSERT INTO trx_alerts (batch_id, date_id, message)
SELECT 
  tpf.batch_id, 
  tpf.date_id, 
...
FROM 
  trx_perf_out tpf 
  JOIN cfg_sor_plan csp ON tpf.operator_id = csp.operator_id
...
async fn process_alerts(
    ...
    for rule in rules {
        if rule.name == "local_vlr_number" {
            ...
            let select_query = r#"
                SELECT batch_id, date_id,COUNT(*) AS value
                FROM trx_perf_out tpf 
                JOIN cfg_sor_plan csp  ON tpf.operator_id = csp.operator_id
                WHERE  operator_count> 0
                AND csp.barring IS TRUE
                AND batch_id = $1
                GROUP BY batch_id, date_id
            "#;
                ...
                .map_err(AppError::Sqlx)?
            {
                ...
                // Insert detailed alerts for barred operators
                sqlx::query(INSERT_BARRED_OPERATOR_ALERTS)
                    .bind(corr_id)
                    .execute(pool)
                    .await
                    .map_err(AppError::Sqlx)?;
                ...
                Logger::info(&format!(
                    "✅ Detailed barred operator alerts inserted for batch_id={}",
                    corr_id
                ));
            }
        }
