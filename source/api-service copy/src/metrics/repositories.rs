...
use crate::metrics::models::{CountryMetric, GlobalMetric, NotifMetric, ValidatedMetricsRequest};
...
impl MetricsRepository {
    ...
    pub async fn get_notif_metrics(
        ...
        let (query, params) = if req.window == 0 {
            (format!("{}", Self::GET_NOTIF_SUM_METRICS_QUERY), vec![])
        } else {
            (format!("{}", Self::GET_NOTIF_DET_METRICS_QUERY), vec![])
        };
