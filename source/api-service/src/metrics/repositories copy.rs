...
impl MetricsRepository {
    ...
    pub async fn get_global_metrics(
        ...
        {
            Some("top") => {
                let size = if let Some(size_str) = req.size {
                    size_str.parse::<i32>().unwrap_or(DEFAULT_SIZE)
                } else {
                    DEFAULT_SIZE
                };
                let query = format!(
                    "{} AND rd.date >= CURRENT_DATE - INTERVAL '{} days' ORDER BY tmg.value DESC LIMIT {}",
    ...
    pub async fn get_country_metrics(
        ...
        let (query, params) = if req.window == 0 {
            let query = format!(
                "{} AND fct.date_id = (SELECT MAX(date_id) FROM trx_metrics_global)",
