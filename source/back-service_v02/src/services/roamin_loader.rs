...
use crate::services::{batch_mgr, file_mgr, lookup};
    ...
    for line in content.lines().map(str::trim).filter(|l| !l.is_empty()) {
        ...
        if in_data_section {
            for caps in re_row.captures_iter(line) {
                let nsub = caps[2].parse().map_err(|e| {
                    AppError::new(format!("Failed to parse nsub '{}': {}", &caps[2], e))
                })?;
                let nsuba = caps[3].parse().map_err(|e| {
...
// File Reading
// ======================================================
fn read_file(file_processed: &FileProcessed) -> Result<RoamInData, AppError> {
...
pub async fn insert_roam_in_batches(pool: &Pool, batches: &[RoamInBatch]) -> Result<u64, AppError> {
    if batches.is_empty() {
        Logger::info("ℹ️ No ROAMIN batches to insert");
        return Ok(0);
    }

    let mut client: Client = pool.get().await?;
    let tx = client.transaction().await?;
    let stmt: Statement = tx
        .prepare(
    ...
    for b in batches {
            ...
            .await
        {
            ...
            Err(e) => Logger::error(&format!(
                "❌ Insert failed: HLR={}, NSUB={}, NSUBA={}, Error: {}",
                b.hlraddr, b.nsub, b.nsuba, e
            )),
        }
    }
...
pub async fn load(
    ...
    for file in files {
        ...
        match process_single_file(&file, &file_processed, pool, batch_mgr, prefix_lookup).await {
            Ok(record_count) => {
                ...
                Logger::info(&format!("✅ {} processed ({} records)", file, record_count));

                let action = source.post_action.as_deref().unwrap_or("delete");
    ...
    Logger::info(&format!(
        "🏁 ROAMIN load complete | Processed: {} | Failed: {} | Total records: {}",
...
async fn process_single_file(
    ...
    if roam_data.records.is_empty() {
        Logger::info(&format!("⚠️ No records found in file: {}", file_name));
        return Ok(0);
    }
    ...
    let batches: Vec<RoamInBatch> = roam_data
        .records
        .into_iter()
        .map(|r| {
            ...
            RoamInBatch {
                ...
                hlraddr: r.hlraddr,
                nsub: r.nsub.to_string(),
                nsuba: r.nsuba.to_string(),
    ...
    insert_roam_in_batches(pool, &batches).await?;
