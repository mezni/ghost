$(document).ready(function () {
  ...
  async function fetchHistoryRoamOutTotal() {
    return fetchData(`${API_BASE}${METRICS_ENDPOINT}?direction=out&dimensions=global&kind=history`);
  }

  async function fetchHistoryRoamOutByCountry() {
    return fetchData(`${API_BASE}${METRICS_ENDPOINT}?direction=out&dimensions=country`);
  }

  async function fetchHistoryRoamOutByOperator() {
    return fetchData(`${API_BASE}${METRICS_ENDPOINT}?direction=out&dimensions=operator`);
  }

  async function initHistoricalChart() {
    try {
      const total = await fetchHistoryRoamOutTotal();
  ...
  async function renderDoughnutCharts() {
    try {
      const [historyByCountry, historyByOperator] = await Promise.all([
        fetchHistoryRoamOutByCountry(),
        fetchHistoryRoamOutByOperator()
      ]);
