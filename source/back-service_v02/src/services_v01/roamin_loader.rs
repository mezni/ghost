...
pub async fn load(
    ...
    Logger::info(&format!(
        "🏁 Completed ROAMIN data load. Processed: {}, Failed: {}, Total records: {}",
...
async fn process_single_file(
    ...
    for record in records {
        let hlraddr_number = record.hlraddr.split('-').nth(1).unwrap_or("").to_string();
        ...
        batches.push(RoamInBatch {
            ...
            nsub: record.nsub.to_string(),
            nsuba: record.nsuba.to_string(),
    ...
    let _inserted_count = insert_roam_in_batches(pool, &batches).await?;
    Logger::debug(&format!("Inserted {} rows ", batches.len()));
