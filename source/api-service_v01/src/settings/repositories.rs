...
use crate::settings::models::{
    Country, CreateCountry, CreateOperator, Operator, UpdateCountry, UpdateOperator,
    Network, CreateNetwork, UpdateNetwork,
};
...
impl OperatorRepository {
    ...
    pub async fn create(pool: &Pool, input: CreateOperator) -> Result<Operator, AppError> {
        ...
        let country_stmt = client
            ...
            .await?;
        let country_row = client
            .query_opt(&country_stmt, &[&input.country_name])
            .await?;
...
impl NetworkRepository {
    ...
    pub async fn get_all(pool: &Pool) -> Result<Vec<Network>, AppError> {
        ...
        let stmt = client
            .prepare(
                "SELECT n.*, c.country_name, o.operator_name
                 FROM cfg_networks n
                 ...
                 ORDER BY c.country_name, o.operator_name, n.plmn_code ASC",
            )
            .await?;
        let rows = client.query(&stmt, &[]).await?;
        Ok(rows.into_iter().map(Self::map_row).collect())
        ...
        let stmt = client
            .prepare(
                "SELECT n.*, c.country_name, o.operator_name
                 ...
                 JOIN cfg_operators o ON n.operator_id = o.operator_id
                 JOIN cfg_countries c ON o.country_id = c.country_id
                 WHERE n.is_valid = TRUE AND n.network_id = $1",
            )
            .await?;
        let row = client.query_opt(&stmt, &[&id]).await?;
        Ok(row.map(Self::map_row))
    ...
    pub async fn create(pool: &Pool, input: CreateNetwork) -> Result<Network, AppError> {
        let client = pool.get().await?;

        // Find operator_id
        let op_stmt = client
            .prepare(
                "SELECT o.operator_id, c.country_name
                 FROM cfg_operators o
                 JOIN cfg_countries c ON o.country_id = c.country_id
                 WHERE o.operator_name = $1 AND c.country_name = $2 AND o.is_valid = TRUE AND c.is_valid = TRUE",
            )
            .await?;
        let op_row = client
            .query_opt(&op_stmt, &[&input.operator_name, &input.country_name])
            .await?;
        let operator_id: i32 = match op_row {
            Some(row) => row.get("operator_id"),
            None => {
                return Err(AppError::Other(
                    "Operator not found for the given country".into(),
                ));
            }
        };
        ...
        let insert_stmt = client
            .prepare(
                "INSERT INTO cfg_networks 
                 (plmn_code, plmn, mcc, mnc, operator_id, tech_2g, tech_3g, tech_lte, created_by)
        ...
        let row = client
            .query_one(
                ...
                &[
                    ...
                    &input.mnc,
                    &operator_id,
                    &input.tech_2g.unwrap_or(false),
                    &input.tech_3g.unwrap_or(false),
                    &input.tech_lte.unwrap_or(false),
                    &input.created_by,
                ],
            )
            .await?;
        let network_id: i32 = row.get("network_id");

        Self::get_by_id(pool, network_id)
            .await?
            .ok_or(AppError::Other("Failed to fetch created network".into()))
    }

    pub async fn update(pool: &Pool, input: &UpdateNetwork) -> Result<Network, AppError> {
        let client = pool.get().await?;
        let stmt = client
            .prepare(
                "UPDATE cfg_networks
                 SET plmn_code = COALESCE($1, plmn_code),
                     plmn      = COALESCE($2, plmn),
                     mcc       = COALESCE($3, mcc),
                     mnc       = COALESCE($4, mnc),
                     tech_2g   = COALESCE($5, tech_2g),
                     tech_3g   = COALESCE($6, tech_3g),
                     tech_lte  = COALESCE($7, tech_lte),
                     updated_at = NOW(),
                     updated_by = $8
                 WHERE network_id = $9
                 RETURNING network_id",
        ...
        Self::get_by_id(pool, network_id)
            .await?
            .ok_or(AppError::Other("Failed to fetch updated network".into()))
    }

    pub async fn delete(pool: &Pool, id: i32) -> Result<(), AppError> {
        let client = pool.get().await?;
        let stmt = client
            .prepare("UPDATE cfg_networks SET is_valid = FALSE WHERE network_id = $1")
            .await?;
        client.execute(&stmt, &[&id]).await?;
        Ok(())
    }
