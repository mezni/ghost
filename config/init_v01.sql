...
FROM ldr_operators lope 
JOIN dim_countries dcou ON dcou.iso_code = lope.country_iso
JOIN dim_operators dope ON dope.operator_name = lope.operator  AND dope.country_id =  dcou.country_id;


INSERT INTO dim_prefixes (country_id, prefix, created_by)
SELECT dcou.country_id, t.prefix, dcou.created_by
FROM dim_countries dcou JOIN
(SELECT 
  iso, 
...
FROM 
  ...
  WHERE prefix_flag = 'X'
) t ON t.iso = dcou.iso_code;

INSERT INTO dim_prefixes (country_id, operator_id, prefix, created_by)
SELECT dope.country_id, dope.operator_id, lpre.prefix, lpre.created_by
FROM ldr_prefixes lpre
JOIN dim_countries dcou ON dcou.country_name = lpre.country
JOIN dim_operators dope ON dope.operator_name = lpre.operator AND dcou.country_id = dope.country_id;
...
FROM 
    sor_plan sor
JOIN 
    dim_operators opr ON sor.operator_id = opr.operator_id
JOIN 
    dim_countries cnt ON opr.country_id = cnt.country_id
JOIN 
    dim_networks net ON opr.operator_id = net.operator_id
JOIN 
    dim_routage_types rtt ON sor.routage_type_id = rtt.routage_type_id
WHERE 
    sor.is_current = TRUE;
...
-- Create the view
CREATE OR REPLACE VIEW v_sor_plan_insert AS
SELECT 
    sor.sor_plan_id,
    cnt.country_name,
    opr.operator_name,
...
CREATE OR REPLACE FUNCTION insert_into_sor_plan()
RETURNS TRIGGER AS $$
DECLARE
    operator_id INTEGER;
    country_id INTEGER;
    routage_type_id INTEGER;
BEGIN
    -- Get the country_id from dim_countries
    SELECT c.country_id INTO country_id
    FROM dim_countries c
    WHERE c.country_name = NEW.country_name;

    -- Get the operator_id from dim_operators
    SELECT o.operator_id INTO operator_id
    FROM dim_operators o
    WHERE o.operator_name = NEW.operator_name AND o.country_id = country_id;

    -- Check if operator_id is found
    IF operator_id IS NULL THEN
        -- If not found, insert a new operator
        INSERT INTO dim_operators (country_id, operator_name)
        VALUES (country_id, NEW.operator_name)
        RETURNING operator_id INTO operator_id;
    END IF;

    -- Get the routage_type_id from dim_routage_types
    SELECT r.routage_type_id INTO routage_type_id
    FROM dim_routage_types r
    WHERE r.routage_type_name = NEW.routage_type_name;

    -- Check if routage_type_id is found
    IF routage_type_id IS NULL THEN
        -- If not found, raise an error
        RAISE EXCEPTION 'Routage type % not found', NEW.routage_type_name;
    END IF;

    -- Insert into sor_plan
    INSERT INTO sor_plan (operator_id, routage_type_id, barring, rate, created_by)
    VALUES (operator_id, routage_type_id, NEW.barring, NEW.rate, 'system');
...
CREATE TRIGGER insert_into_sor_plan_trigger
INSTEAD OF INSERT ON v_sor_plan_insert
FOR EACH ROW
EXECUTE PROCEDURE insert_into_sor_plan();